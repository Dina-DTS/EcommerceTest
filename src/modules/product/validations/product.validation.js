...
// Define a reusable schema for image validation
...
// Refactored addProductShema (unchanged)
export const addProductShema = Joi.object({
  ...
  imageCover: Joi.array().items(imageSchema).required(),
  images: Joi.array().items(imageSchema).required(),
});
// Refactored updateProductSchema
export const updateProductSchema = Joi.object({
  ...
  createdby: Joi.string().hex().length(24).optional(),
  imageCover: Joi.array().items(imageSchema).optional(),
